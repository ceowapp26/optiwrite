...
import { useShopifySession } from '@/hooks/useShopifyAuth';
import { useAppBridge } from '@/providers/AppBridgeProvider';

interface ShopifSignInButtonProps {
  shop: string;
  host: string;
}

export default function ShopifSignInButton({ shop, host }: ShopifSignInButtonProps) {
  const { signIn } = useShopifySession({ shop, host });
