...
interface Metadata {
  ...
  readingTime?: number
  isOriginal?: boolean
  contentRating?: 'beginner' | 'intermediate' | 'advanced'
  views?: number
  likes?: number
  shares?: number
}
...
export interface Output extends BaseEntity{
 ...
 inventory: {
   ...
   managedBy?: 'manual' | 'automatic'
 }
 videos?: Array<{
   url: string
   thumbnail?: string
   type: 'product_overview' | 'tutorial' | 'review'
 }> 
 features: string[]
 ...
 variants?: Array<Omit<Product, 'variants'>>
}
...
export type CATEGORY = 'BLOG' | 'ARTICLE' | 'PRODUCT';
...
interface ProductVariant {
  ...
  option2: string | null;
  option3: string | null;
  presentment_prices: ProductVariantPresentmentPriceSet[]; 
  position: number;
  price: string;
  requires_shipping: boolean;
  sku: string;
...
interface ProductVariantPresentmentPriceSet {
  price: Money;  
...
export type CONTENT = PRODUCT | ARTICLE;
